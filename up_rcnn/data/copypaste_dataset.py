...
class CopyPasteDataset(Dataset):
    ...
    def __getitem__(self, index):
        image_s = cv2.imread(random.choice(self.paths))
        image_q = cv2.imread(random.choice(self.paths))
        image_k = cv2.imread(random.choice(self.paths))

        x, y, w, h = self.get_position(image_s, self.cut_scale)
        image_s = image_s[y:y + h, x:x + w, :]
        boxes_s = [[0, 0, w - 1, h - 1]]

        image_q, boxes_q = self.copy_and_paste(image_s, image_q)
        image_k, boxes_k = self.copy_and_paste(image_s, image_k)
        ...
        image_k, boxes_k = self.box_trans(image_k, torch.tensor(boxes_k, dtype=torch.float32))

        image_s = self.image_trans(image_s)
        image_q = self.image_trans(image_q)
        image_k = self.image_trans(image_k)
    ...
    def copy_and_paste(self, foreground, background):
        composed = background.copy()
        x, y, w, h = self.get_position(background, self.paste_scale)
        trans = self.get_aug((h, w))
        foreground_trans = np.asarray(trans(Image.fromarray(foreground)))
        composed[y:y + h, x:x + w, :] = foreground_trans
